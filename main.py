...
def webhook():
    ...
    if "message" in data:
        ...
        if state == "greeting":
            ...
            reply = (
                "Если я правильно понял вас, суть ситуации в том, что...\n"
                "Это действительно то, на чём вы хотели бы сосредоточиться? Или вы бы сформулировали иначе?"
            )
            user_state[chat_id] = "desired_result"

        elif state == "desired_result":
            reply = (
                "А если представить, что всё сложилось хорошо — как бы это выглядело для вас?\n"
                "Что бы вы хотели в итоге почувствовать, видеть, иметь?"
            )
            user_state[chat_id] = "contradiction"

        elif state == "contradiction":
            reply = (
                "Давайте посмотрим вместе: нет ли тут какого-то внутреннего напряжения или противоречия?\n"
                "Например, вы хотите одного, но одновременно вас что-то сдерживает или мешает."
            )
            user_state[chat_id] = "resolve_contradiction"

        elif state == "resolve_contradiction":
            reply = (
                "Если не спешить и немного поразмышлять — как, по-вашему, можно было бы приблизиться к тому, чего вы хотите, "
            ...
            user_state[chat_id] = "solution"

        elif state == "solution":
            reply = (
                "Что может быть реалистичным решением в вашей ситуации?\n"
                "Что можно изменить — в действиях, подходе, ожиданиях — чтобы приблизиться к желаемому результату?"
            )
            user_state[chat_id] = "reflect_solution"

        elif state == "reflect_solution":
            reply = (
                "Если я правильно понял вас, вы склоняетесь к этому решению.\n"
            ...
            user_state[chat_id] = "action_step"

        elif state == "action_step":
            reply = (
                "Мы с вами проделали хороший путь: обозначили суть ситуации, наметили желаемый результат, "
            ...
            user_state[chat_id] = "closing"

        elif state == "closing":
            reply = (
                "Любой разговор — это способ простроить новые связи: между мыслями, чувствами, решениями.\n"
